...
fn default_plugin_store_dir(app: &AppHandle) -> Result<PathBuf, String> {
    ...
    Ok(crate::data_dir::resolve_data_dir_with_mode(default_data_dir, config_dir.as_deref()).data_dir.join("plugins"))
}

fn default_agent_store_dir(app: &AppHandle) -> Result<PathBuf, String> {
    let default_data_dir = app.path().app_data_dir().map_err(|e| e.to_string())?;
    let config_dir = app.path().app_config_dir().ok();
    let data_dir_resolution = crate::data_dir::resolve_data_dir_with_mode(default_data_dir, config_dir.as_deref());
