...
fn main() {
    ...
    make_bindings_rust_177_compatible(&out_path);
}

fn make_bindings_rust_177_compatible(out_path: &Path) {
    let Ok(bindings) = std::fs::read_to_string(out_path) else {
        return;
    };
    let fixed = bindings.replace("unsafe extern \"C\"", "extern \"C\"");
    if fixed != bindings {
        std::fs::write(out_path, fixed).unwrap_or_else(|_| panic!("Could not write to {out_path:?}"));
    }
}
