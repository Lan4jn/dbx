...
async fn stream_select_query_text(
    ...
    #[cfg(feature = "legacy-tokio-postgres-0713")]
    let stream = futures::stream::iter(
        client.simple_query(sql).await.map_err(pg_error_to_string)?.into_iter().map(Ok::<_, tokio_postgres::Error>),
    );
    #[cfg(not(feature = "legacy-tokio-postgres-0713"))]
