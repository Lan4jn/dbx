...
pub fn duckdb_object_source_with_attached(
    ...
    let mut stmt = con
        ...
        .map_err(|e| e.to_string())?;
    let mut rows = stmt
        .query_map(duckdb::params![database.as_str(), schema, name], |row| row.get::<_, String>(0))
        .map_err(|e| e.to_string())?;
...
fn duckdb_column_comments(
    ...
    let Ok(rows) = stmt.query_map(duckdb::params![database, schema, table], |row| {
...
async fn list_objects_once(
    ...
    let (mysql_limit, mysql_offset) =
        if filter.map_or(true, |value| value.trim().is_empty()) { (limit, offset) } else { (None, None) };
