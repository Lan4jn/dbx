...
const autoRefreshIntervals = computed(() => {
  if (!props.autoRefresh) return [];
  if (props.autoRefresh.onApplyCustomInterval) return [...props.autoRefresh.intervalOptions];
  return dataGridToolbarIntervalOptions(props.autoRefresh.intervalOptions, props.autoRefresh.intervalSeconds);
});
const customIntervalInput = ref("15");
const customIntervalInputRef = ref<HTMLInputElement | null>(null);

watch(
  () => props.autoRefresh?.customIntervalSeconds,
  (seconds) => {
    if (seconds == null || document.activeElement === customIntervalInputRef.value) return;
    customIntervalInput.value = String(seconds);
  },
  { immediate: true },
);

async function applyCustomInterval() {
  if (!props.autoRefresh?.onApplyCustomInterval || customIntervalInput.value.trim() === "") return;
  await props.autoRefresh.onApplyCustomInterval(customIntervalInput.value);
}

async function activateCustomIntervalInput() {
  await applyCustomInterval();
  await nextTick();
  customIntervalInputRef.value?.focus({ preventScroll: true });
  customIntervalInputRef.value?.select();
}

async function onCustomIntervalKeydown(event: KeyboardEvent) {
  ...
  <div class="flex shrink-0 items-center gap-1 px-1">
    ...
    <DropdownMenu v-if="isDataGridToolbarCapabilityVisible(autoRefresh)">
      ...
      <DropdownMenuContent align="end" class="w-40">
        ...
        <div v-if="autoRefresh?.onApplyCustomInterval" class="relative flex cursor-text select-none items-center gap-1.5 whitespace-nowrap rounded-md px-1.5 py-1 text-sm outline-hidden focus-within:bg-accent hover:bg-accent" @pointerdown.stop @click.stop.prevent="void activateCustomIntervalInput()">
          <Check v-if="autoRefresh.customIntervalSelected" class="h-3.5 w-3.5 shrink-0" />
          <span v-else class="h-3.5 w-3.5 shrink-0" />
          <span class="shrink-0">{{ autoRefresh.customLabel }}</span>
          <input
            ref="customIntervalInputRef"
            v-model="customIntervalInput"
            type="text"
            inputmode="numeric"
            pattern="[0-9]*"
            ...
            :aria-label="autoRefresh.customInputLabel"
            @pointerdown.stop
            @click.stop
            @keydown.stop="onCustomIntervalKeydown"
            @blur="void applyCustomInterval()"
          />
          <span class="shrink-0 text-xs text-muted-foreground">{{ autoRefresh.customUnitLabel }}</span>
        </div>
      </DropdownMenuContent>
    </DropdownMenu>
