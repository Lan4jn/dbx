...
function Resolve-WebView2Source {
  ...
  $candidateRoots = @(
    ...
    (Join-Path $repoRoot "src-tauri\target-win7-x64\$LegacyTarget\release\bundle\portable"),
    (Join-Path $repoRoot "src-tauri\target-win7-x64\$LegacyTarget\release\bundle\old-release")
  )
...
if ($buildLegacy) {
  Move-StalePortableArtifacts -PortableRoot (Join-Path $repoRoot "src-tauri\target-win7-x64\$LegacyTarget\release\bundle\portable") -KeepNames $legacyKeepNames -ArchiveStamp $cleanupStamp
}
